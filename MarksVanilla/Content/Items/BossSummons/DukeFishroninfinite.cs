...
{
    public class DukeFishroninfinite : ModItem
    {
        ...
        public override bool CanUseItem(Player player) {
			...
			// Duke Fishron only summonable in ocean biome and if one does not already exist
			return !NPC.AnyNPCs(NPCID.DukeFishron) && Main.LocalPlayer.ZoneBeach;
		}
        ...
        public override void AddRecipes() { //use chaining method to add the recipe for this item
			CreateRecipe()
				.AddIngredient(ItemID.TruffleWorm, 15) //this should be replaced with a custom item that drops upon breaking bulbs
