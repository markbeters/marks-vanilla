...
{
    public class EoWinfinite : ModItem
    {
        ...
        public override bool CanUseItem(Player player) {
			...
			// Eater of Worlds is summonable only in corruption and if one does not exist already
			// Condition is a class not stored as bool, using "Main.LocalPlayer.ZoneCorrupt" works as a boolean
			return !NPC.AnyNPCs(NPCID.EaterofWorldsHead) && !NPC.AnyNPCs(NPCID.EaterofWorldsBody) && !NPC.AnyNPCs(NPCID.EaterofWorldsTail) && Main.LocalPlayer.ZoneCorrupt;
		}
        ...
        public override void AddRecipes() { //use chaining method to add the recipe for this item
			CreateRecipe()
				.AddIngredient(ItemID.WormFood, 10)
