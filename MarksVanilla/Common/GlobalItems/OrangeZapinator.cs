...
namespace MarksVanilla.Common.GlobalItems
{
	...
	// 
	public class OrangeZapinatorChanges : GlobalItem
	{
		// Only instancing this for Orange Zapinator item
		public override bool AppliesToEntity(Item item, bool lateInstantiation) {
			return item.type == ItemID.ZapinatorOrange; // grab the int id for Orange Zapinator
		}
        ...
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.setBonus == "Refunds 4 mana per attack")
            {
                ...
                player.statMana -= (int)(16 * player.manaCost); //this takes the base mana cost of 16 and applies proper mana reduction to it 
